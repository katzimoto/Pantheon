...
Change-specific reading recipes — which documents to read, in order, for a
common kind of change, and which to consult only conditionally — are in
`docs/architecture/recipes.md`. They are conditional retrieval guidance, not
canonical inventory; the domain tables above are the inventory. Follow the
matching recipe when a change matches one, and read the listed documents in
order instead of reading the tree.
...
inventory is maintained. Conditional change-specific reading recipes belong in
`docs/architecture/recipes.md`, not as new per-domain sections here.