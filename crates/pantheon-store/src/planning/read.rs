...
impl Store {
    ...
    /// Every Task in a Goal's graph, ordered by id.
    pub fn tasks_for_goal(&self, goal_id: &str) -> Result<Vec<TaskRecord>, StoreError> {
        self.read(|conn| {
            ...
            for row in rows {
                let (id, phase, created_graph_revision, spec_digest, revision, active_run_id) =
                    row?;
