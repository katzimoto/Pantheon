...
//! the daemon's internals. It depends on `pantheon-operator-protocol` alone,
//! which leaves it structurally unable to bypass the daemon rather than merely
//! discouraged from doing so — `scripts/check-crate-deps.sh` enforces that.
//!
